...
export default abstract class BaseService <TServiceEvent> {
  ...
  // #region Events
  /**
   * Subscription TTL requested from the speaker (in seconds). A shorter TTL makes the
   * speaker re-confirm the SID more often, so a SID the speaker has internally forgotten
   * surfaces as a renew failure (412) sooner, narrowing the stall window.
   */
  private static readonly SubscriptionTimeoutSeconds = 300;

  private static get SubscriptionTimeoutHeader(): string {
    return `Second-${BaseService.SubscriptionTimeoutSeconds}`;
  }

  /**
   * Renew cadence (in ms). Kept at roughly half the subscription TTL so a renew always
   * lands before the speaker expires the SID.
   */
  ...
  private async subscribeForEvents(): Promise<boolean> {
    ...
    if (this.eventRenewInterval === undefined) {
      ...
      }, BaseService.RenewIntervalMs);
    }
    SonosEventListener.DefaultInstance.RegisterSubscription(sid, this);
    this.subscribedAt = Date.now();
    this.lastNotifyAt = undefined;

    return true;
  }

  /**
   * Stamp the time a NOTIFY was delivered for this service. Called by the SonosEventListener,
   * the single point every NOTIFY passes through, so it is the source of NOTIFY-liveness truth.
   *
   * @remarks Should not be called by anything other than SonosEventListener.
   */
  ...
  /**
   * The time (ms) of the most recent NOTIFY, exposed for diagnostics. Undefined when no
   * NOTIFY has been received since the last (re)subscribe.
   */
  public get LastNotifyAt(): number | undefined {
